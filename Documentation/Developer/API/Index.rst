...
Since `mysqlreport` 2.0.0 there is a completely rewritten API to add your
...
If you want to extend pages of `mysqlreport` with further infoboxes you have
to add some lines to the Service.yaml of your extension.
...
Create a new PHP class which extends the `AbstractInfoBox` class
of `mysqlreport`:
...
You have to set the `pageIdentifier` to show your infobox on the right view
in backend module.

Set a `title`. It will be used as the title in the infobox.

The content of the infobox has to be rendered in the `renderBody` method. Over
the `$page` argument you have access to all the status and variable values of
your MySQL/MariaDB server.

..  hint::

    Do not add HTML tags into your content within `renderBody` method. The
    content will be passed through `htmlspecialchars`. Please apply a Pull
...
If a server variable is not available for a server, it may help to hide the
infobox in backend module.
